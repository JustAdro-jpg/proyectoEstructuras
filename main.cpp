...
Grafo* cargarGrafoDesdeArchivo(const string& nombreArchivo)
{
    ...
    Hash hashVertices;           // para registrar nombres únicos de ciudades
    Lista8<Arista> listaAristas; // lista temporal de aristas
    ...
    while (getline(in, linea))
    {
        ...
        while (ss >> nombreDestino)
        {
            ...
            // Crear arista (origen, destino) como strings
            Arista a(nombreOrigen, nombreDestino);
            listaAristas.insertarFin(a);
            cantidadAristas++;
        }
    }
    ...
    cout << "Cantidad de aristas: " << cantidadAristas << endl;

    // Crear grafo definitivo (ahora solo recibe cantidadVertices)
    Grafo* g = new Grafo(cantidadVertices);

    // Pasar las aristas de la lista al grafo
...
int main()
{
    ...
    // Ejecutar Karger varias veces
    int repeticiones = 100; // pueden ajustar este número
